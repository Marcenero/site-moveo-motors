...
function numeroNormalizado(schema) {
...
const estadoIpvaSchema = z.preprocess((valor) => {
    if (typeof valor !== "string") {
        return valor;
    }
...
const outrasInfosSchema = z.preprocess((valor) => {
...
const camposVeiculo = {
    ...
    final_placa: numeroNormalizado(z
        .number({
    ...
    estado_ipva: estadoIpvaSchema,
    preco: numeroNormalizado(z
        .number({
    ...
    outras_infos: outrasInfosSchema,
};
...
export const idVeiculoSchema = z.strictObject({
    id: z
        .string({
        error: "ID inválido.",
    })
...
export function formatarErrosZod(erro) {
    return erro.issues.map((issue) => ({
        campo: issue.path.length > 0
            ? issue.path.join(".")
            : "body",
